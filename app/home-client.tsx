...
const categoryStyles = ["bg-[#f7ded7]", "bg-[#dfe8d9]", "bg-[#e8e1ef]", "bg-[#eee5c8]", "bg-[#d9e4ed]"];
const categorySymbols = ["♪", "✦", "◒", "⌁", "✺"];

export default function HomeClient({ events, categories }: { events: PublicEvent[]; categories: PublicCategory[] }) {
  const router = useRouter();
  const [search, setSearch] = useState("");
  const [category, setCategory] = useState("All");
  const [saved, setSaved] = useState<string[]>([]);
  const featured = events.find((event) => event.featured) ?? events[0];
  const curatedEvents = useMemo(() => category === "All" ? events.slice(0, 3) : events.filter((event) => event.category === category).slice(0, 3), [category, events]);

  const handleSearch = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    router.push(search.trim() ? `/events?search=${encodeURIComponent(search.trim())}` : "/events");
  };
  ...
  return (
    <main className="overflow-hidden bg-[#f7f7f4]">
      <section className="grain relative bg-[#dfe8d9]">
        ...
        <div className="relative mx-auto grid max-w-[1280px] items-center gap-12 px-5 pb-14 pt-12 sm:pb-20 sm:pt-16 lg:grid-cols-[0.92fr_1.08fr] lg:gap-14 lg:px-8 lg:pb-[104px] lg:pt-20">
          <div className="reveal max-w-[570px]">
            ...
            <p className="mt-7 max-w-[450px] text-[16px] leading-7 text-[#536056] sm:text-[17px]">Discover gatherings, big nights out and everything in between. Your calendar is about to get interesting.</p>
          ...
          <div className="reveal reveal-delay-2 relative mx-auto w-full max-w-[570px] lg:ml-auto">
            {featured ? <div className="relative aspect-[0.98/1] overflow-hidden rounded-[30px] bg-[#242725] shadow-[10px_14px_0_rgba(36,39,37,0.13)] sm:aspect-[1.08/1]">
              <Image alt={featured.title} className="object-cover opacity-85" fill priority sizes="(max-width: 1024px) 92vw, 50vw" src={featured.image} />
              ...
              <div className="absolute left-5 top-5 flex items-center gap-2 rounded-full bg-white/15 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.14em] text-white backdrop-blur-md"><span className="h-1.5 w-1.5 rounded-full bg-[#f16d55]" /> {featured.featured ? "Featured event" : featured.category}</div>
              <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8"><div className="mb-3 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#f3c4b8]"><CalendarDays size={14} /> {featured.dateLong}</div><h2 className="font-display max-w-[390px] text-[clamp(2.35rem,4vw,4rem)] leading-[0.91] text-white">{featured.title}</h2><div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-white/20 pt-4 text-[12px] text-white/70"><span className="flex items-center gap-1.5"><MapPin size={14} /> {featured.venueName}</span><span className="font-semibold text-white">{featured.priceLabel ?? "Tickets to be announced"}</span></div></div>
              <Link aria-label={`View ${featured.title}`} className="absolute bottom-6 right-6 flex h-11 w-11 items-center justify-center rounded-full bg-[#f16d55] text-[#242725] hover:scale-105 hover:bg-white" href={`/events/${featured.slug}`}><ArrowUpRight size={19} /></Link>
            </div> : <EmptyState title="A blank canvas, for now." description="Published events will appear here as soon as your event team adds them." action={<Link className="rounded-full bg-[#242725] px-5 py-3 text-[12px] font-semibold text-white" href="/admin/login">Open the event workspace</Link>} />}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1280px] px-5 pb-16 pt-20 sm:pt-24 lg:px-8 lg:pb-24" id="discover">
        ...
        {categories.length > 0 && <div className="mt-10 flex gap-2 overflow-x-auto pb-2 [scrollbar-width:none]"><button className={`shrink-0 rounded-full px-5 py-2.5 text-[12px] font-semibold ${category === "All" ? "bg-[#242725] text-white" : "border border-[#e0e3dd] bg-white text-[#717a73] hover:border-[#b9c0b9]"}`} onClick={() => setCategory("All")} type="button">All picks</button>{categories.map((item) => <button className={`shrink-0 rounded-full px-5 py-2.5 text-[12px] font-semibold ${category === item.name ? "bg-[#242725] text-white" : "border border-[#e0e3dd] bg-white text-[#717a73] hover:border-[#b9c0b9]"}`} key={item.name} onClick={() => setCategory(item.name)} type="button">{item.name}</button>)}</div>}
        {curatedEvents.length > 0 ? <div className="mt-6 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{curatedEvents.map((event, index) => <div className={`reveal reveal-delay-${index + 1}`} key={event.slug}><EventCard event={event} onSave={(slug) => setSaved((items) => items.includes(slug) ? items.filter((item) => item !== slug) : [...items, slug])} saved={saved.includes(event.slug)} /></div>)}</div> : <div className="mt-8"><EmptyState title={category === "All" ? "The calendar is taking a breath." : `Nothing in ${category}, yet.`} description="There are no published events in this view. Try another category or check back soon." action={<Link className="rounded-full bg-[#242725] px-5 py-3 text-[12px] font-semibold text-white" href="/events">Browse the calendar</Link>} /></div>}
      </section>

      <section className="border-y border-[#e7e9e4] bg-white" id="categories"><div className="mx-auto max-w-[1280px] px-5 py-16 lg:px-8 lg:py-20"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-3 text-[11px] font-bold uppercase tracking-[0.18em] text-[#e15f49]">Follow your curiosity</p><h2 className="font-display text-[clamp(2.5rem,4vw,3.8rem)] leading-[0.94]">What are you<br /><span className="text-[#929b94]">in the mood for?</span></h2></div><p className="max-w-[250px] text-[13px] leading-5 text-[#7b847d]">Browse the categories your event team has published.</p></div>{categories.length > 0 ? <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{categories.map((item, index) => <Link className={`group relative overflow-hidden rounded-[20px] ${categoryStyles[index % categoryStyles.length]} p-5 transition-transform hover:-translate-y-1`} href={`/events?category=${encodeURIComponent(item.name)}`} key={item.name}><span className="flex h-12 w-12 items-center justify-center rounded-full bg-white/60 font-display text-[28px] text-[#242725]">{categorySymbols[index % categorySymbols.length]}</span><h3 className="mt-12 text-[16px] font-semibold tracking-[-0.03em] text-[#242725]">{item.name}</h3><div className="mt-1 flex items-center justify-between text-[11px] text-[#677168]"><span>{item.count} {item.count === 1 ? "event" : "events"}</span><ArrowUpRight className="opacity-0 transition-opacity group-hover:opacity-100" size={15} /></div></Link>)}</div> : <div className="mt-10"><EmptyState title="Categories will bloom here." description="Once an event is published, its category will become part of this little map." /></div>}</div></section>

      <section className="mx-auto grid max-w-[1280px] gap-10 px-5 py-16 sm:py-24 lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:px-8" id="how-it-works"><div><div className="mb-7 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#f16d55] text-[#242725] shadow-[4px_4px_0_#242725]"><Sparkles size={26} /></div><p className="mb-3 text-[11px] font-bold uppercase tracking-[0.18em] text-[#e15f49]">The evently edit</p><h2 className="font-display text-[clamp(2.7rem,4.6vw,4.4rem)] leading-[0.91] text-[#242725]">Good plans<br />start <span className="text-[#e15f49]">here.</span></h2><p className="mt-6 max-w-[360px] text-[14px] leading-6 text-[#747d76]">The best memories are often the ones you almost didn’t make. Finding your next yes should feel effortless.</p><Link className="mt-7 inline-flex items-center gap-2 text-[13px] font-semibold text-[#242725] hover:text-[#e15f49]" href="/events">Explore the calendar <ArrowRight size={16} /></Link></div><div className="grid gap-3 sm:grid-cols-2"><div className="relative min-h-[240px] overflow-hidden rounded-[24px] bg-[#202321] p-6 text-white sm:row-span-2 sm:min-h-[400px]"><div className="absolute -right-10 -top-10 h-48 w-48 rounded-full border-[28px] border-[#f16d55]/80" /><div className="absolute bottom-5 left-6 right-6"><p className="text-[11px] uppercase tracking-[0.15em] text-white/45">01 / Discover</p><h3 className="mt-2 text-[22px] font-semibold tracking-[-0.04em]">Follow the feeling</h3><p className="mt-2 max-w-[240px] text-[12px] leading-5 text-white/55">Find the places, people and ideas you didn’t know you were looking for.</p></div></div><div className="relative min-h-[190px] overflow-hidden rounded-[24px] bg-[#eee5c8] p-6 text-[#242725]"><div className="absolute right-5 top-5 flex h-11 w-11 items-center justify-center rounded-full border border-[#242725]/20"><CalendarDays size={19} /></div><p className="text-[11px] uppercase tracking-[0.15em] text-[#242725]/45">02 / Save</p><h3 className="mt-12 text-[20px] font-semibold tracking-[-0.04em]">Make a little room</h3></div><div className="relative min-h-[190px] overflow-hidden rounded-[24px] bg-[#e8e3ef] p-6 text-[#242725]"><div className="absolute right-5 top-5 flex h-11 w-11 items-center justify-center rounded-full border border-[#242725]/20"><Users size={19} /></div><p className="text-[11px] uppercase tracking-[0.15em] text-[#242725]/45">03 / Go</p><h3 className="mt-12 text-[20px] font-semibold tracking-[-0.04em]">Bring your people</h3></div></div></section>
