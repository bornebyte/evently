...
export async function GET(_request: Request, { params }: { params: Promise<{ reference: string }> }) {
  ...
  return NextResponse.json({ reference: booking.reference, attendeeName: booking.attendeeName, attendeeEmail: booking.attendeeEmail, ticketToken: booking.ticketToken, total: Number(booking.total), currency: booking.currency, status: booking.status, event: booking.event, ticket: booking.items[0]?.ticketType.name ?? "Ticket", quantity: booking.items.reduce((sum, item) => sum + item.quantity, 0) });
}
