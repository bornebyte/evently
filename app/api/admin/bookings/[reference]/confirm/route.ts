...
export async function POST(_request: Request, { params }: { params: Promise<{ reference: string }> }) {
  ...
  if (booking.status === "CONFIRMED") return NextResponse.json({ ok: true, reference, emailSent: Boolean(booking.emailSentAt) });

  const updated = await prisma.booking.update({ where: { id: booking.id }, data: { status: "CONFIRMED", verifiedAt: new Date() }, include: { event: true, items: { include: { ticketType: true } } } });
  try {
